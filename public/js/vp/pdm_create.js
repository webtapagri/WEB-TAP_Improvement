      ...
      $.ajax({
        ...
        success : function(json) {
          if(json == 0) {
            ...
            $('#iTanggalMasukKerja').val(json.join_date);
            $('#iTanggalLahir').val(json.dob);
            $('#yob').val(json.age_year);
            $('#mob').val(json.age_month);
            $('#dob').val(json.age_day); //iTanggalLahir,
            $('#iUmur').val(json.age);

            switch (json.pss) {
              case '7':
                $('input[name=iSuratTeguran]').val('Surat Teguran');
                $("#iMasaBerlaku").val(moment(json.eff_date_pss).format('DD-MMM-YYYY'));
              break;
              case '8':
                $('input[name=iSuratTeguran]').val('Surat Peringatan 1');
                $("#iMasaBerlaku").val(moment(json.eff_date_pss).format('DD-MMM-YYYY'));
              break;
              case '9':
                $('input[name=iSuratTeguran]').val('Surat Peringatan 2');
                $("#iMasaBerlaku").val(moment(json.eff_date_pss).format('DD-MMM-YYYY'));
              break;
              case '10':
                $('input[name=iSuratTeguran]').val('Surat Peringatan 3');
                $("#iMasaBerlaku").val(moment(json.eff_date_pss).format('DD-MMM-YYYY'));
              break;
              default:
                $('input[name=iSuratTeguran]').val('');
                $("#iMasaBerlaku").val('');
            }
            ...
            $('#ba_code').val(json.werks);
            //$('#iStatusKaryawanNew').select2().val(json.STATUS).trigger("change");
            $('#iAkhirKontrak').val(json.expire_date_contract);
            $('#akhkont').val(json.expire_date_contract);
            $('#iKontrakKe').val(json.contract_to);
            $('#kontrakke').val(json.contract_to);
            $('#iPrevKontrak').val(json.period_probation);
            $('#prevkon').val(json.period_probation);
            $('#spk_sk_old').val(json.spk_sk);
            $("#comp_code_old").val(json.comp_code);
          
            $('#iPerusahaanOld').val(json.comp_name);
            $('#iBisnisAreaOld').val(json.werks_name);
            $('#iJabatanOld').val(json.job_name);
            $('#iGolonganOld').val(json.gol_name);
            $('#iStatusKaryawanOld').val(json.status_name);
          
            //$('#iPerusahaanNew').select2().val(json.comp_code).trigger("change");
            //$('#iJabatanNew').select2().val(json.job_code).trigger("change");
            //if (json.golongan) $('#iGolonganNew').select2().val(json.golongan).trigger("change");
          
            //cek mandatory data
            sessionStorage.setItem("dob", json.dob);
            sessionStorage.setItem("no_ktp", json.no_ktp);
            sessionStorage.setItem("no_kk", json.no_kk);
            sessionStorage.setItem("spk_sk", json.spk_sk);
            sessionStorage.setItem("entry_date", json.entry_date);
            sessionStorage.setItem("bpjs_ketenagakerjaan", json.bpjs_ketenagakerjaan);
            sessionStorage.setItem("bpjs_kesehatan", json.bpjs_kesehatan);
            ...
            $('#ba_code_old').val(json.werks);
            $('#job_code_old').val(json.job_code);
            $('#job_type_old').val(json.golongan);
            $('#emp_status_old').val(json.status);
...
function cekkontrakke(i) {
  ...
  if(i == 1) {
    if(kontrakke >= 2 ) {//|| (kontrakke == 1 && prevkont == 24) ) {
      ...
      $("#iJenisPerubahan option[value=CT01]").attr('disabled','disabled');
      $("#iJenisPerubahan option[value=CT02]").removeAttr('disabled');
      showLog('kontrakke');
    }
  }
}
...
function clearForm(){
    $('#iTanggalMasukKerja, #iNama, #iTanggalLahir, #iUmur, #iAtt3yAgo, #iAtt2yAgo, #iAtt1yAgo, #iProd3yAgo, #iProd2yAgo, #iProd1yAgo').val(null);
    $('#iJenisPerubahan, #iNomorPtk, #emp_status_old, #stat_new, #sisaKontrak, #iNextKontrak, #ba_code_old,#ba_code').val(null) //reset rule jadi 01CRE
    $('#iPerusahaanOld, #iBisnisAreaOld, #iJabatanOld, #iGolonganOld, #iStatusKaryawanOld, #prevkon, #kontrakke, #akhkont, #iAkhirKontrak, #iKontrakKe, #iNextKontrak').val(null);
}